...
[ Raw, Unstructured Data ] (PDF, JPG, SVG, etc.)
                  ...
                  ▼
┌──────────────────────────────────────────────────┐
│ 1. INGESTION: FRONTEND (React)                   │
│    - File capture via browser interface          │
└──────────────────────────────────────────────────┘
                  │ (REST API / Upload)
                  ▼
┌──────────────────────────────────────────────────┐
│ 2. FILTRATION: BACKEND (FastAPI)                 │
│    - 🛡️ Magic Bytes validation & Malware block  │
│    - 🧼 Auto-sanitization (EXIF, initial PII)    │
└──────────────────────────────────────────────────┘
                  │ (Safe data returned to UI)
                  ▼
┌──────────────────────────────────────────────────┐
│ 3. INSPECTION: FRONTEND (Operator Vault)         │
│    - ✏️ Manual visual redaction (Canvas)         │
│    - 🧠 Context enrichment and operator notes    │
└──────────────────────────────────────────────────┘
                  │ (Approval / Export)
                  ▼
┌──────────────────────────────────────────────────┐
│ 4. AGGREGATION: BACKEND (Data Schema Engine)     │
│    - Object relationship consolidation           │
│    - JSON structure and ZIP archive generation   │
└──────────────────────────────────────────────────┘
                  │
                  ▼
       [ Standardized, Clean Data Package ]
```
...
[ Surowe, Nieustrukturyzowane Dane ] (PDF, JPG, SVG, itp.)
                  ...
                  ▼
┌──────────────────────────────────────────────────┐
│ 1. WEJŚCIE: FRONTEND (React)                     │
│    - Przechwycenie pliku przez przeglądarkę      │
└──────────────────────────────────────────────────┘
                  │ (REST API / Upload)
                  ▼
┌──────────────────────────────────────────────────┐
│ 2. FILTRACJA: BACKEND (FastAPI)                  │
│    - 🛡️ Walidacja Magic Bytes i blokada Malware │
│    - 🧼 Auto-czyszczenie (EXIF, wstępne PII)     │
└──────────────────────────────────────────────────┘
                  │ (Bezpieczne dane wracają do UI)
                  ▼
┌──────────────────────────────────────────────────┐
│ 3. INSPEKCJA: FRONTEND (Krypta Operatora)        │
│    - ✏️ Ręczna redakcja wizualna (Canvas)        │
│    - 🧠 Wzbogacenie o kontekst i notatki         │
└──────────────────────────────────────────────────┘
                  │ (Zatwierdzenie / Export)
                  ▼
┌──────────────────────────────────────────────────┐
│ 4. AGREGACJA: BACKEND (Data Schema Engine)       │
│    - Konsolidacja relacji obiektów               │
│    - Generowanie struktury JSON i archiwum ZIP   │
└──────────────────────────────────────────────────┘
                  │
                  ▼
   [ Ustandaryzowana, Czysta Paczka Danych ]
```
